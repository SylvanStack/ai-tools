...
class UserDal(DalBase):
    ...
    async def reset_current_password(self, user: entity.User, data: vo.ResetPwd) -> None:
        """
        重置密码
        :param user:
        :param data:
    ...
    async def update_current_info(self, user: entity.User, data: vo.UserUpdateBaseInfo) -> Any:
        """
        更新当前用户基本信息
        :param user:
        :param data:
...
class RoleDal(DalBase):
    ...
    async def get_select_datas(self) -> list:
        ...
        return [vo.RoleOptionsOut.model_validate(i).model_dump() for i in queryset.all()]
...
class MenuDal(DalBase):
    ...
    def generate_router_tree(self, menus: list[entity.Menu], nodes: filter, name: str = "") -> list:
        ...
        data = []
        for root in nodes:
            router = vo.RouterOut.model_validate(root)
    ...
    def generate_tree_list(self, menus: list[entity.Menu], nodes: filter) -> list:
        ...
        data = []
        for root in nodes:
            router = vo.MenuTreeListOut.model_validate(root)
...
class DeptDal(DalBase):
    ...
    def generate_tree_list(self, depts: list[entity.Dept], nodes: filter) -> list:
        ...
        data = []
        for root in nodes:
            router = vo.DeptTreeListOut.model_validate(root)
...
class TestDal(DalBase):
    ...
    async def test_left_join(self):
        ...
        # 原查询语句：
        # DeptAlias = aliased(entity.Dept)
        # sql = select(self.model).where(self.model.is_delete == false())
        # sql = sql.options(contains_eager(self.model.depts, alias=DeptAlias))
        # sql = sql.outerjoin(entity._auth_user_depts, self.model.id == entity._auth_user_depts.c.user_id)
        # sql = sql.outerjoin(
        #     DeptAlias,
        #     and_(DeptAlias.id == entity._auth_user_depts.c.dept_id, DeptAlias.owner == "张伟")
        # )
        # self.db.expire_all()
        # queryset = await self.db.scalars(sql)
        # result = queryset.unique().all()
    ...
    async def relationship_where_operations_any(self):
        ...
        """
        print("==============================any 方法使用案例1=========================================")
        # 用户表（entity.User）与 部门表（Dept）为多对多关系
    ...
    async def relationship_where_operations_has(self):
        ...
        """
        print("==============================has 方法使用案例1=========================================")
        # 用户（entity.User）与 帮助问题（entity.Issue）为多对一关系
